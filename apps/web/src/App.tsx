    ...
    <div className="flex h-screen flex-col bg-[var(--color-bg)]">
      ...
      <header className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-2 bg-[var(--color-surface)]">
        ...
        <div className="flex items-center gap-4">
          {/* Language Selector — fix: use <label> instead of <span> */}
          <div className="flex items-center gap-2">
          ...
          {/* Run Button — fix: aria-label with state context */}
          <button
            onClick={handleRunCode}
            disabled={!connected || isRunning}
            aria-label={isRunning ? "Running code, please wait" : "Run code"}
            aria-busy={isRunning}
          ...
          >
            {isRunning ? (
              <>
                {/* fix: aria-hidden on decorative spinner SVG */}
                <svg aria-hidden="true" className="animate-spin h-3.5 w-3.5 text-slate-400" fill="none" viewBox="0 0 24 24">
              ...
              <>
                <span aria-hidden="true" className="text-xs">▶</span> Run
              </>
            )}
          </button>

          {/* Download Button — fix: aria-label (title alone is insufficient) */}
          <button
            onClick={handleDownload}
            disabled={!ytext}
            aria-label={`Download ${FILE_NAMES[language]}`}
          ...
          {/* Connection Indicator — fix: aria-label on dot, not color-only status */}
          <div
            className="flex items-center gap-2 border-l border-[var(--color-border)] pl-4"
            role="status"
            aria-live="polite"
            aria-label={connected ? "Connection status: Connected" : "Connection status: Disconnected"}
          >
            <span
              aria-hidden="true"
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <aside aria-label="File explorer and editor settings" className="w-56 shrink-0 border-r border-[var(--color-border)] bg-[var(--color-surface)] p-3 flex flex-col justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500" aria-hidden="true">
              Explorer
            </p>
            {/* fix: make file item keyboard-focusable with role */}
            <nav aria-label="Open files">
              <div className="mt-3 space-y-1">
                <div
                  ...
                  tabIndex={0}
                  aria-current="true"
                  aria-label={`Current file: ${FILE_NAMES[language]}`}
                  className="rounded px-2 py-1 text-sm font-medium text-tessera-400 bg-tessera-500/10 border border-tessera-500/20 focus:outline-none focus:ring-2 focus:ring-tessera-500"
                >
                  📄 {FILE_NAMES[language]}
                </div>
              </div>
          ...
          <div className="border-t border-[var(--color-border)] pt-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3" aria-hidden="true">
              Editor Settings
            </p>
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-slate-300" id="font-size-label">Font Size</span>
                <div className="flex items-center gap-1.5" role="group" aria-labelledby="font-size-label">
                  {/* fix: descriptive aria-label on font size buttons */}
                  <button
                  ...
                  <span
                    className="text-xs font-mono font-medium text-slate-200 min-w-[28px] text-center"
                    aria-live="polite"
        ...
        {/* Editor */}
        <main aria-label="Code editor" className="flex-1 overflow-hidden">
          {ytext && awareness ? (
            <CollaborativeEditor
              ytext={ytext}
              awareness={awareness}
            ...
            <div
              role="status"
              aria-live="polite"
              aria-label="Connecting to collaboration server"
              className="flex h-full flex-col items-center justify-center gap-3 text-slate-500 font-medium bg-[var(--color-bg)]"
            >
              <svg aria-hidden="true" className="animate-spin h-8 w-8 text-tessera-400" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
      ...
      {/* Output panel — fix: aria-live so screen readers announce new output */}
      <section
        aria-label="Execution output"
        className="h-56 shrink-0 border-t border-[var(--color-border)] bg-[var(--color-surface)] flex flex-col p-3 overflow-hidden"
      >
        <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-2 mb-2">
          ...
          {output && (
            <div className="flex gap-4 text-xs font-medium" aria-label={`Status: ${output.status}, Duration: ${output.durationMs}ms`}>
        ...
        {/* fix: aria-live="polite" announces new output to screen readers */}
        <div
          aria-live="polite"
          aria-atomic="false"
          aria-label="Code execution output"
          className="flex-1 overflow-y-auto font-mono text-xs p-2 rounded bg-[var(--color-bg)] border border-[var(--color-border)]"
        >
          {isRunning ? (
            <span role="status" className="text-tessera-400 animate-pulse">Running execution sandbox...</span>
          ) : output ? (
            <div className="space-y-1 whitespace-pre-wrap">
              {output.stdout && <div className="text-emerald-300">{output.stdout}</div>}
              {output.stderr && <div className="text-rose-400 font-semibold" role="alert">{output.stderr}</div>}
